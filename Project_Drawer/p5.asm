...
waitMS:
	mov ax,3h
	int 33h
	cmp bx, 10b 	; if left button is down
	je readColor
	cmp bx, 01h 	; if right button is down
	je draw
...
readColor:
; read color from screen on mouse place to AL
	mov bh, 0h
	mov ah, 0dh       ; get color of a single pixel.
	int 10h
	mov [color+5], ax    ; AL = pixel color
	jmp waitMS
